    ...
    <div className="space-y-6">
            ...
            <div key={item.product.id} className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground">
                  {item.product.price.toFixed(3)} DT x {item.quantity}
                </p>
              </div>
              <p className="font-medium">{(item.product.price * item.quantity).toFixed(3)} DT</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-secondary/50 p-6 rounded-lg">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between">
            <span>Livraison</span>
            <span>{shippingPrice === 0 ? 'Gratuit' : `${shippingPrice.toFixed(3)} DT`}</span>
          </div>
          <div className="flex justify-between">
            <span>Taxe (15%)</span>
            <span>{tax.toFixed(3)} DT</span>
          </div>
