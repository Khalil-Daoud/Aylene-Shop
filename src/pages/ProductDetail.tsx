...
const ProductDetail = () => {
  ...
  const normalizeProduct = (data: {
    ...
    const resolvedId = data.id ?? data._id ?? data.productId ?? '';
    return {
  ...
  useEffect(() => {
    const fetchProductAndRelated = async () => {
      ...
      setIsLoading(true);
      try {
        const productResponse = await fetch(ENDPOINTS.PRODUCTS.GET_ONE(id));
        ...
        if (normalizedProduct.category) {
          const url = new URL(ENDPOINTS.PRODUCTS.GET_ALL);
          url.searchParams.append('category', normalizedProduct.category);

          const relatedResponse = await fetch(url.toString());
          if (relatedResponse.ok) {
            const relatedData = await relatedResponse.json();
            const normalizedRelated = Array.isArray(relatedData)
              ? relatedData.map(normalizeProduct)
              : [];
            setRelatedProducts(
              normalizedRelated
      ...
      <div className="min-h-screen pt-24 pb-16 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            Nous récupérons les informations du produit.
          </p>
        </div>
      </div>
    );
