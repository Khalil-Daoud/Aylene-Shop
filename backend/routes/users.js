...
const authMiddleware = (req, res, next) => {
  ...
  console.log("Received token (users): - users.js:9", token);
  if (!token) {
    return res
      .status(401)
  ...
  try {
    ...
    console.log("Decoded token (users): - users.js:21", decoded);
    if (!decoded.isAdmin) {
      return res
        .status(403)
    ...
    console.error("Token verification error (users): - users.js:31", error);
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("Erreur lors de la récupération des utilisateurs: - users.js:56", error);
...
router.get("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("Get user error: - users.js:70", error);
    res.status(500).json({ message: "Erreur serveur" });
  }
});
...
router.post("/", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("Create user error: - users.js:143", error);
...
router.put("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("Erreur lors de la mise à jour de l'utilisateur: - users.js:191", error);
    res.status(500).json({ message: "Erreur serveur", error: error.message });
  }
});
...
router.delete("/:id", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("Delete user error: - users.js:207", error);
    res.status(500).json({ message: "Erreur serveur", error: error.message });
  }
});
